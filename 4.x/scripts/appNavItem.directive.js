(function () {
    ...
    function AppNavItemDirective ($compile, $location, $route) {
        return {
            restrict: 'E',
            replace: true,
            ...
            link: function (scope, element) {
                ...
                if (angular.isString(scope.item.directive)) {
                    ...
                    injectContent('.appNav__item-directive', directiveHtml);
                }
            ...
            controller: function ($scope, $location, $injector, rxVisibility, Permission, urlUtils) {
                ...
                $scope.getUrl = function (url) {
                    ...
                    // Check if we have a definition of NAV_ITEM_PREFIX, if so let's retrieve it and return the given
                    // URL appended to the prefix.  This allows applications like origin to prefix nav items, while
                    // not messing with nav items in the demo/documentation.
                    //
                    // _This is *NOT* meant for general consumption, this is strictly for the Origin Project_
                    // _This will eventually be deprecated and removed_
                    //
                ...
                $scope.getTarget = function () {
                    // Check if we have a definition of NAV_ITEM_TARGET, if so let's retrieve it and enable the target
                    // attr on the nav item.  This allows applications like origin to give a target to it's nav items,
                    // while not messing with nav items in the demo/documentation.
